@TestForecast
Feature: As a weatherForecast service provider, I should be able to fetch the whether details from the application successfully
  ...
  Scenario Outline: Verify the weatherForecast Application whether user can able to lunch the application for mentioned city
    ...
    @NagativeScenario
  Scenario Outline: Verify the weatherForecast Applicif user enter incorrect city name then user should get error message
      When I open the url http://localhost:3000/
      When I enter the <cityName> for weatherForecast
  ...
  Scenario Outline: Verify that user can able to hidden the forecast data.
    ...
    Then I should be able to launch the application with header "Five Day Weather Forecast for"
    When I enter the <cityName> for weatherForecast
    Then User should be able to hidden forecast data "<day>" "<cityName>"
...
#    When I open the url http://localhost:3000/
#    Then I should be able to launch the application with header "Five Day Weather Forecast for"
#    When I enter the perth for weatherForecast
